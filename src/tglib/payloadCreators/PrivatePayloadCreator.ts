...
import {
  type IPrivateAnimationPayload,
  type IPrivateAudioPayload,
  type IPrivateCbQueryPayload,
  type IPrivateCommandPayload,
  type IPrivateContextPayload,
  type IPrivateDocumentPayload,
  type IPrivateEditedMessagePayload,
  type IPrivateLocationPayload,
  type IPrivatePhotoPayload,
  type IPrivateStickerPayload,
  type IPrivateTextPayload,
  type IPrivateVideNotePayload,
  type IPrivateVideoPayload,
  type IPrivateVoicePayload,
  type MediaPayload
} from '../tgTypes/messagePayload/contextPayloadTypes';
import {
  type IPayloadCreator,
  PayloadCreatorFunc,
  type PRIVATE_PAYLOAD_CREATORS_HOLDER
} from '../tgTypes/messagePayload/payloadCreator';
import { PAYLOAD_TYPES } from '../tgTypes/messagePayload/messagePayloadTypes';
import { type Context } from 'telegraf';
import { Message, Update, type User } from 'typegram';
...
export default class PrivatePayloadCreator extends PrivateUpdateHandler implements IPayloadCreator {
  readonly decoratorCreatorFunctionByUpdateType: PRIVATE_PAYLOAD_CREATORS_HOLDER<IPrivateContextPayload> = {
  ...
  public create(updateType: PRIVATE_UPDATE_TYPES, context: Context<Update>): IPrivateContextPayload | null {
    const functionCreator: undefined | PayloadCreatorFunc<IPrivateContextPayload> =
      this.decoratorCreatorFunctionByUpdateType[updateType];

    if (functionCreator === undefined) {
      console.log(`Creation of this kind of payload (${updateType}) not yet supported`);

      return null;
    }

    const result: IPrivateContextPayload = functionCreator(context);
  ...
  protected override onCallbackQuery(context: Context<Update>): IPrivateCbQueryPayload {
    ...
    const defaultObj: IPrivateContextPayload = this.getDefaultPayload(
      context,
